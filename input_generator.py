...
# Really don't think this was worth it
...
# not tested or proofread
def path_drive_car_to_homes(G, H, s):
    total_path = [s]
    H.remove(s)
    while H:
        next_node = random.choice(H)
        H.remove(next_node)
        prev_node = total_path[-1]
        path = nx.shortest_path(G, source=prev_node, target=next_node)
        # Strip the first element of the path since it's already in total_paths
