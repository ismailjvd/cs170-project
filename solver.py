...
def create_path_mapping_everyone_walks(G, H, s):
    n = len(G.nodes())
    path = [s]
...
def remove_two_cycles_from_path(path, homes, G):
    ...
    while change_needed and len(new_path) > 2:
        ...
        # print(path)
        # print(new_path)
        # we will no longer continue if keeping the two-cycles on the previous iteration is more efficient
...
def solve(list_of_locations, list_of_homes, starting_car_location, adjacency_matrix, params=[]):
    ...
    try:
        ...
        naive_path, naive_dropoffs = create_path_mapping_everyone_walks(G, home_indices, start_index)
        ...
        path, dropoffs = create_path_mapping_everyone_walks(G, home_indices, start_index)
